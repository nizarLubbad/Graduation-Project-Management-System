...
export function KanbanBoard({ teamId }: KanbanProps) {
  ...
  // Load tasks when teamId changes
  useEffect(() => {
    if (!teamId) return;
    const stored = localStorage.getItem(storageKey);
    if (stored) setColumns(JSON.parse(stored));
    else setColumns(initialData); // reset if no data
  }, [teamId, storageKey]);

  // Save tasks to localStorage
  const saveToStorage = (cols: Column[]) => {
    if (!teamId) return;
    localStorage.setItem(`kanbanTasks_${teamId}`, JSON.stringify(cols));
  };

  const addTask = (colId: string) => {
    if (!newTask[colId]?.title || !user || !teamId) return;
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      const updatedColumns = columns.map(c => c.id === column.id ? { ...c, tasks: newTasks } : c);
      setColumns(updatedColumns);
      saveToStorage(updatedColumns);
    } else {
              ...
              >
                ...
                {col.tasks.map((task, index) => (
                  <Draggable key={task.id} draggableId={task.id} index={index}>
                    {(provided, snapshot) => (
                      <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className={`mb-2 ${snapshot.isDragging ? "opacity-70" : ""}`}>
                        <Card className="p-3 border bg-white rounded-md shadow-sm">
                          {editingTask?.id === task.id ? (
                            <div className="flex flex-col gap-2">
                              ...
                              <select value={editingTask.priority} onChange={e => setEditingTask({ ...editingTask, priority: e.target.value as "high" | "medium" | "low" })} className="border rounded px-2 py-1 text-sm">
                            ...
                            <>
                              ...
                              <div className="flex justify-between items-center text-xs">
                                <span className={`px-2 py-0.5 rounded-full font-medium ${priorityBadge(task.priority)}`}>{task.priority}</span>
                                <div className="flex items-center gap-1"><Calendar className="h-3 w-3" />{task.dueDate}</div>
                              </div>
                              <div className="flex justify-between items-center mt-2 text-xs text-gray-500">
                                <div className="flex items-center gap-1"><User className="h-3 w-3" /> Added by: {task.createdBy}</div>
                                <div className="flex gap-2">
                                  <Button size="icon" variant="ghost" onClick={() => setEditingTask(task)}><Pencil className="h-4 w-4" /></Button>
                                  <Button size="icon" variant="ghost" onClick={() => deleteTask(col.id, task.id)}><Trash2 className="h-4 w-4 text-red-500" /></Button>
                                </div>
                              </div>
                            </>
                          )}
                ...
                <div className="mt-3 flex flex-col gap-2">
                  ...
                  <select value={newTask[col.id]?.priority || "medium"} onChange={e => setNewTask({ ...newTask, [col.id]: { ...newTask[col.id], priority: e.target.value as "high" | "medium" | "low" } })} className="border rounded px-2 py-1 text-sm">
                    <option value="high">High</option>
                    <option value="medium">Medium</option>
                    <option value="low">Low</option>
                  </select>
                  <Button onClick={() => addTask(col.id)}><Plus className="h-4 w-4 mr-1" /> Add Task</Button>
                </div>
              </div>
            )}
          </Droppable>
