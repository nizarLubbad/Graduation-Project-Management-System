    ...
    <div className="h-screen flex flex-col bg-gray-100">
      ...
      <header className="h-16 bg-gray-200 shadow flex items-center justify-between px-4 sm:px-6 relative z-30">
        <div>
          ...
          {user?.team?.teamId && (
            <p className="text-xs text-gray-500">
              Team: <span className="font-semibold ">{user.team.teamName}</span>
            </p>
          )}
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="hidden lg:inline bg-black text-white px-3 py-1 rounded hover:bg-gray-800 text-sm"
          >
            Sign Out
          </button>
        </div>
        ...
        {menuOpen && (
          <div className="absolute top-full left-0 right-0 bg-white shadow-md border-b border-gray-300 flex flex-col z-50">
            {menuItems.map(item => (
              <NavLink
                key={item.path}
      ...
      <div className="flex flex-1">
        ...
        {canAccessDashboard ? (
          ...
          <main className="flex-1 flex items-center justify-center bg-white mt-16 lg:mt-0">
